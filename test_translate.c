...
static void test_charseq_length(void)
{
    ...
    CHECK_SIZE( charseq_length("a-y"), 25 );
    CHECK_SIZE( charseq_length("z-ab"), 1 );
    CHECK_SIZE( charseq_length("a--z"), 1 );
    CHECK_SIZE( charseq_length("a-z"), 26 );
    CHECK_SIZE( charseq_length("\\"), 1 );
    CHECK_SIZE( charseq_length("z-a"), 0 );
    CHECK_SIZE( charseq_length("Z-a"), 8 );
    CHECK_SIZE( charseq_length("A-Z"), 26 );
    CHECK_SIZE( charseq_length("A-z"), 58 );
    CHECK_SIZE( charseq_length("c-c"), 1 );
    CHECK_SIZE( charseq_length(""), 0 );
    CHECK_SIZE( charseq_length("\n"), 1 );
    CHECK_SIZE( charseq_length("\?"), 1 );
    CHECK_SIZE( charseq_length("A\?"), 2 );
}
...
static void test_expand_charseq(void)
{
    ...
    check_expand( "A-C", "ABC" );
    check_expand( "a-z", "abcdefghijklmnopqrstuvwxyz" );
    check_expand( "A-z", "ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz" );
    check_expand( "A--C", "C" );
    check_expand( "c-a", "" );
    check_expand( "A-Cf", "ABCf" );
    check_expand( "A--Cf", "Cf" );
    check_expand( "GA--C", "GC" );
    check_expand( "GA-C", "GABC" );
    check_expand( "1-3", "123" );
    check_expand( "A-C1-5", "ABC12345" );
    check_expand( "A--C", "C" );
    check_expand( "A\?", "A?" );
    check_expand( "Y-\\n", "YZ[\\n" );
}   
...
static void test_translate_char(void)
{
    ...
    CHECK_CHAR( translate_char('y', "xyz", "X2Z"), '2' );
    CHECK_CHAR( translate_char('z', "1ez", "1e5"), '5' );
    CHECK_CHAR( translate_char('z', "12z", "135"), '5' );
    CHECK_CHAR( translate_char('d', "cdd", "XYZ"), 'Y' );
}
...
static void test_translate(void)
{
    ...
    check_tr( "hELLO, WORLD!", "abcdef-zABCDEF-Z", "ABCD3F-Zabcd3f-z", "H3llo, world!" );
    check_tr( "ab\x09/\x7F z", "b \x09\x7F/", "B+/*t", "aB/t*+z" );
    check_tr( "abcde", "abcde", "badce", "badce" );
